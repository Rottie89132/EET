		...
		<div :class="!installed ? ' mt-20' : 'mt-14'" class="overflow-hidden">
			<div :class="!installed ? 'top-[4rem] fixed' : 'top-0 pt-14 md:top-0 md:-mt-5 fixed'" class="w-full z-10 px-6 bg-white">
				<div class="mb-4">
					...
					<button v-if="installed" class="bg-[#4e995b] text-[#cfe7d3] p-2 px-4 rounded-xl mt-[0.15rem]" @click="logout">
			...
			<div :class="!installed ? ' w-screen mt-[4.4rem] md:mt-[5.5rem] ' : ' fixed w-screen max-h-[72vh] mt-[8.5rem]  '" class="overflow-auto pb-5">
						...
						<div class="mt-1">
							<div v-if="itemsReserveringen?.length > 0" class="flex gap-2 items-center justify-between">
								<div class="flex gap-2 items-center">
									...
									<span class="bg-[#4e995b] font-semibold text-[#cfe7d3] p-1 px-3 text-xs rounded-lg">{{ itemsReserveringen[0].datum.split("-").reverse().join("-") }}</span>
								</div>
	...
	<Modal :title v-model:active="active" v-model:activeDelay="activeDelay">
		<div v-if="title == 'Instelling'">
			...
			<Form @submit="handleRequest" :validation-schema="schema" v-slot="{ meta }">
				...
				<FieldAvatar label="Avatar" name="avatar" v-model:preview="Avatar" v-model:previewArray="AvatarArray" />

				<div v-if="result" class="flex gap-2 mt-6">
					<hr class="my-3" />
				...
				<div class="flex gap-2 items-center">
					<div class="bg-[#de4747] text-white p-1 px-4 rounded-lg" @click="toonDelete">
		...
		<div v-if="title == 'Verwijder'">
			<p class="-mt-2 text-gray-600">Weet u zeker dat u uw account wilt verwijderen? <br /></p>
			<p class="text-red-600 text-sm leading-4 mt-4">Alle gegevens worden verwijderd en kunnen niet worden hersteld. inclusief alle restaurants en reserveringen onder dit account!</p>

			<div v-if="result" class="flex gap-2 mt-2 mb-2">
				<hr class="my-3" />
				<p :class="result.statusCode != 200 ? 'text-red-600' : 'text-green-800'" class="text-sm leading-4 -ml-1">
					{{ result.message }}
				</p>
			...
			<div class="flex gap-2 w-full items-center">
				<button class="bg-[#de4747] text-white p-1 px-4 rounded-lg" @click="deleteAccount">
					<span v-if="displayLoading" class="">
...
<script setup lang="ts">
	...
	const toonDelete = async () => {
		activeDelay.value = false;
		title.value = "Verwijder";
		setTimeout(() => {
	...
	const deleteAccount = async () => {
		...
		if (error.value) result.value = error.value.data;
		if (!error.value) {
			result.value = data.value.message;
			setTimeout(() => {
				navigateTo("/home");
			}, 4000);
		}
