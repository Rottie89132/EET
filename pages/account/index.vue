<template>
	<div class="h-screen">
		...
		<div :class="!installed ? ' mt-20' : 'mt-14'" class="overflow-hidden">
												...
												<div class="flex justify-between gap-2">
													<div class="w-full">
														<div class="flex items-center justify-between gap-2 w-full">
															...
															<div v-if="!item.error" class="flex items-center gap-2 text-right pb-2 overflow-auto">
			...
			<!-- <div :class="!installed ? ' fixed w-screen bg-red-500 ' : ' fixed w-screen overflow-auto max-h-[72vh] pb-6 '" class="grid px-6 mt-[5.5rem] grid-cols-1 xl:grid-cols-3 gap-2 pb-2 md:mb-6 overflow-auto">
				<div class="bg-[#4e995b] text-white p-4 rounded-xl mt-2">
					<div class="flex items-center justify-between">
						<div>
							<h2 class="font-semibold flex items-center gap-1 text-lg">
					...
					<div class="flex items-center w-full justify-between">
						<div class="w-full">
							<details :open="openDetail === 'reservations'" class="font-semibold flex items-center justify-between gap-1 text-lg">
								...
								<p class="text-sm -my-1 font-normal">
									<span v-if="items.length > 10" class="text-xs opacity-80"> Lijst met alle reserveringen onder dit account </span>
									<span v-else class="text-xs opacity-80"> Je hebt geen reserveringen onder dit account </span>
								</p>
								<div class="grid gap-2 mt-3 font-normal">
									<div v-if="items.length > 10" v-for="item in 10" class="">
										<Reserveringen plaats="Rotterdam" restaurant="Restaurant" datum="2022-12-12" tijd="12:00" naam="Naam" :rating="4" :aantal="3" keuken="Nederlands" prijs="$$" id="234567"> </Reserveringen>
									</div>
									<div v-else>
										<button class="bg-gray-300 p-[0.35rem] px-3 text-xs text-black rounded-lg">Maak een reservering</button>
									</div>
								</div>
							</details>
						</div>
						...
						<div class="w-full">
							<details :open="openDetail === 'restaurants'" class="font-semibold flex items-center justify-between gap-1 text-lg">
								...
								<div class="grid gap-2 mt-3 font-normal">
									...
									<div v-else>
										<button class="bg-gray-300 p-[0.35rem] px-3 text-xs text-black rounded-lg">Voeg een restaurant toe</button>
									</div>
								</div>
							</details>
						</div>
...
<!-- <div>
											<div class="flex items-center gap-2">
												<h3 class="font-semibold text-md">{{ item.naam }}</h3>
											</div>
											<div class="flex items-center gap-2">
												<span class="bg-gray-300 p-1 flex items-center px-3 text-xs text-black rounded-lg">
													<icon name="ic:sharp-star" size="1em" class="text-black"></icon>
													{{ item.beoordeling }}
												</span>
												
												<span class="bg-gray-300 p-1 px-3 text-xs text-black rounded-lg">{{ item.aantal_reserveringen }} reserveringen</span>
											...
											<div class="flex gap-2 mt-2">
												<button class="bg-gray-300 p-1 px-3 text-xs text-black rounded-lg">Beheer</button>
												<button class="bg-gray-300 p-1 px-3 text-xs text-black rounded-lg">Bewerk</button>
												<button class="bg-gray-300 p-1 px-3 text-xs text-black rounded-lg">Verwijder</button>
											</div>
										</div> -->
