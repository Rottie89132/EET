<template>
	<field :name="name" v-slot="{ handleChange, handleBlur, field, meta }">
		...
		<div :class="meta.touched && !meta.valid ? ' mt-2 mb-5 border-[#B92538] focus:border-[#B92538] ' : isDraggingOver ? ' border-black' : ' border-gray-300'" class="w-full flex items-center justify-center relative border-2 border-dashed rounded-lg p-2 mb-3 mt-2 min-h-36" @dragover.prevent="dragOver" @dragleave.prevent="dragLeave" @drop="fileDropped(handleChange, $event)" ref="dropzone">
			<input :multiple="multipe" type="file" aria-label="file" autocomplete="current-file" ref="fileInput" class="absolute inset-0 w-full h-full opacity-0 z-50" @change="fileSelected(handleChange, $event)" @blur="handleBlur" />
...
<script setup lang="ts">
	...
	const displayPreview = (file: File) => {
		...
		reader.onload = () => {
			const img = new Image();
			img.onload = () => {
				if (img.width !== 1728 || img.height !== 668) return;
