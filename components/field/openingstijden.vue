			...
			<div class="mb-2 text-sm grid grid-cols-2 gap-4">
				<div>
					...
					<div class="flex items-center gap-2">
						<div>
							<input class="mt-1 p-1 bg-gray-100 border-gray-100 border rounded-md outline-none" @change="setWeekdayOpenTimes(handleChange, $event)" type="time" />
						</div>
						<div>
							<input class="mt-1 p-1 bg-gray-100 border-gray-100 border rounded-md outline-none" @change="setWeekdayCloseTimes(handleChange, $event)" type="time" />
						</div>
					</div>
				</div>
				<div>
					...
					<div class="flex items-center gap-2">
						<div>
							<input class="mt-1 p-1 bg-gray-100 border-gray-100 border rounded-md outline-none" @change="setWeekendOpenTimes(handleChange, $event)" type="time" />
						</div>
						<div>
							<input class="mt-1 p-1 bg-gray-100 border-gray-100 border rounded-md outline-none" @change="setWeekendCloseTimes(handleChange, $event)" type="time" />
						</div>
					</div>
				</div>
			</div>
			<div>
				...
				<div class="mb-2 text-sm grid grid-cols-2 gap-4">
					<div v-for="(time, index) in openingTimes.filter((time) => ['Maandag', 'Dinsdag', 'Woensdag', 'Donderdag', 'Vrijdag'].includes(time.dag))" :key="time.dag">
						...
						<div class="flex items-center gap-2">
							<div>
								<input :class="meta.touched && !meta.valid && time.open == undefined ? '  border-[#B92538] focus:border-[#B92538] ' : ' border-gray-300'" class="mt-1 p-1 bg-white border rounded-md outline-none" @change="pick(handleChange)" v-model="time.open" type="time" />
							</div>
							<div>
								<input :class="meta.touched && !meta.valid && time.sluit == undefined ? ' border-[#B92538] focus:border-[#B92538] ' : ' border-gray-300'" class="mt-1 p-1 bg-white border rounded-md outline-none" v-model="time.sluit" @change="pick(handleChange)" type="time" />
							</div>
						</div>
					</div>
				</div>
