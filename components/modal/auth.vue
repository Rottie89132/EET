				...
				<div tabindex="0" ref="modal">
					<Transition :name="ismoble ? 'modalDelay' : 'modalSlide'">
						<div :ref="ismoble ? 'modalDelay' : 'modalSlide'" v-if="activeDelay">
							<div
								class="p-8 md:h-screen h-[60vh] rounded-2xl md:rounded-none bg-white w-screen md:max-w-[28vw]">
								...
								<Form @submit="handleRequest" :validation-schema="schema" v-slot="{ meta }">
									...
									<FieldInput type="password" label="Wachtwoord" name="wachtwoord"
										v-model="password" />
									<span @click="forgotPassword" class="opacity-80 cursor-pointer">Wachtwoord
										vergeten</span>
									...
									<div class="mb-4 mt-4 flex gap-2">
										<button
											...
											<span v-if="displayLoading" class="">
												<icon class=" animate-spin" name="pajamas:repeat" size="1.3rem">
												</icon>
											</span>
											<span v-else>Inloggen</span>
										</button>
										<div @click="handleRegister"
											class="w-full text-center p-2 bg-gray-100 cursor-pointer rounded-md hover:bg-gray-200 focus:outline-none">
											Registreren</div>
									</div>
								</Form>
